...
4. Fill in the pull request template, including how you verified the change. CI runs the same gate plus a Prettier check on the pull request and attaches the built site as an artifact. `main` only takes pull requests whose `gate` check passed on the latest commit, with every review thread resolved; the maintainer listed in `.github/CODEOWNERS` is requested automatically.
