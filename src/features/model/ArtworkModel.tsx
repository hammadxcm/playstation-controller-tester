...
export function ArtworkModel({
  ...
  useEffect(() => {
    ...
    setAnchors({
      ...
      ls,
      rs,
      mute: box(byId(spec.anchors.mute)),
      gripL: box(byId(spec.anchors.gripL)),
      gripR: box(byId(spec.anchors.gripR)),
    })
  }, [prepared, spec])

  useControllerRig(wrapper, { travel: spec.travel, key: anchors, source })
  ...
  useFrame((f) => {
    frame.current = {
      ...
      rx: f.axes[2] ?? 0,
      ry: f.axes[3] ?? 0,
      l2: f.buttons[STD.l2]?.value ?? 0,
      r2: f.buttons[STD.r2]?.value ?? 0,
    }
  })
  useHidState((s) => {
    hid.current = s
    ...
    return onRaf(() => {
      ...
      pts.forEach((g, i) => {
        ...
        if (on && t)
          g.setAttribute(
            'transform',
            `translate(${(tp.x + t.x * tp.w).toFixed(1)} ${(tp.y + t.y * tp.h).toFixed(1)})`,
          )
      })
      if (s) {
  ...
  return (
    <div
      ref={wrapper}
      className={`model art art-${spec.style} ${compact ? 'compact' : ''} ${hero ? 'art-hero' : ''}`}
      data-kind={kind}
          ...
          <>
            {spec.lightbar === 'strips' && (
              <g className="m-lightbar">
                <path
                  ...
                  fill="none"
                  strokeLinecap="round"
                  d={`M${a.touchpad.x - fs * 0.2},${a.touchpad.y + fs * 1.4} Q${a.touchpad.x - fs * 0.9},${a.touchpad.y + a.touchpad.h / 2} ${a.touchpad.x + fs * 1.2},${a.touchpad.y + a.touchpad.h - fs * 0.4}`}
                />
                <path
                  className="m-lb-glow"
                  strokeWidth={fs * 0.9}
                  fill="none"
                  strokeLinecap="round"
                  d={`M${a.touchpad.x + a.touchpad.w + fs * 0.2},${a.touchpad.y + fs * 1.4} Q${a.touchpad.x + a.touchpad.w + fs * 0.9},${a.touchpad.y + a.touchpad.h / 2} ${a.touchpad.x + a.touchpad.w - fs * 1.2},${a.touchpad.y + a.touchpad.h - fs * 0.4}`}
                />
                <path
            ...
            {spec.lightbar === 'top' && (
              <g className="m-lightbar">
                <rect
                  className="m-lb-glow"
                  x={a.touchpad.x + a.touchpad.w * 0.18}
                  y={a.touchpad.y - fs * 1.1}
                  width={a.touchpad.w * 0.64}
                  height={fs * 0.8}
                  rx={fs * 0.4}
                  strokeWidth={fs * 0.6}
                  fill="none"
              ...
              <g className="m-leds">
                {[-2, -1, 0, 1, 2].map((i) => (
                  <g key={i} className="m-led" data-led={i + 2}>
                    <circle
                      cx={a.touchpad.x + a.touchpad.w / 2 + i * fs * 0.8}
                      cy={a.touchpad.y + a.touchpad.h + fs * 1.1}
                      r={fs * 0.3}
                      fill="#fff"
                      opacity={0.35}
                    />
            ...
            {a.gripL && (
              <ellipse
                className="m-heat"
                data-heat="l"
                cx={a.gripL.x + a.gripL.w * 0.3}
                cy={a.gripL.y + a.gripL.h * 0.72}
                rx={a.gripL.w * 0.35}
                ry={a.gripL.h * 0.3}
                fill="url(#m-heat-art)"
              />
            ...
            {a.gripR && (
              <ellipse
                className="m-heat"
                data-heat="r"
                cx={a.gripR.x + a.gripR.w * 0.7}
                cy={a.gripR.y + a.gripR.h * 0.72}
                rx={a.gripR.w * 0.35}
                ry={a.gripR.h * 0.3}
                fill="url(#m-heat-art)"
              />
              ...
              <g className="m-values">
                {[lsC, rsC].map(([cx, cy, r], i) => (
                  <g key={i}>
                    ...
                    <circle
                      cx={cx! + (i ? values.rx : values.lx) * spec.travel}
                      cy={cy! + (i ? values.ry : values.ly) * spec.travel}
                      r={fs * 0.12}
                      fill="#f25757"
                    />
                    <text
                      ...
                      className="m-text"
                    >
                      X {(i ? values.rx : values.lx).toFixed(3)}
                    </text>
                    <text
                      x={cx}
                      y={cy! + r! + fs * 2.4}
                      ...
                      className="m-text"
                    >
                      Y {(i ? values.ry : values.ly).toFixed(3)}
                    </text>
                  </g>
                ))}
                <rect
                ...
                <text
                  ...
                  className="m-text"
                >
                  L2 {values.l2.toFixed(2)}
                </text>
                <text
                  x={a.touchpad.x + a.touchpad.w}
                  y={a.touchpad.y - fs * 0.3}
                  fontSize={fs * 0.8}
                  textAnchor="end"
                  className="m-text"
                >
                  R2 {values.r2.toFixed(2)}
                </text>
              </g>
            )}
          </>
