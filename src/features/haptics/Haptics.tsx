...
export function Haptics() {
  ...
  const fire = async (s: number, w: number, l = 0, r = 0) => {
    ...
    const res = await rumble(pad ? getGamepad(pad.index) : null, {
      duration,
      strong: s,
      weak: w,
      leftTrigger: l,
    ...
    <div className="stack">
      <Card>
        ...
        <p className="small muted" style={{ textAlign: 'center' }}>
          The model shakes and the grips glow for whichever motor is running.
        </p>
      </Card>
      <div className="grid-2">
        <Card
          title="Rumble (Gamepad API)"
          right={
            <div className="row">
              <ProgressRing ref={ring} size={28} stroke={4} />
              {caps.dual ? (
                <Badge tone="good">supported</Badge>
              ) : (
                <Badge tone="bad">not supported here</Badge>
              )}
            </div>
          }
        >
          <Slider
            label="Strong motor (left, low-frequency)"
            value={strong}
            onChange={setStrong}
            format={(v) => `${Math.round(v * 100)} %`}
          />
          <Slider
            label="Weak motor (right, high-frequency)"
            value={weak}
            onChange={setWeak}
            format={(v) => `${Math.round(v * 100)} %`}
          />
          ...
          />
          <div className="row">
            <Button primary disabled={!caps.dual} onClick={() => fire(strong, weak)}>
              Play
            </Button>
            {PRESETS.map((p) => (
              <Button
            ...
            <Button
              small
              onClick={() => {
                fx.emit('stop', undefined)
                stopRing.current?.()
                void stopRumble(pad ? getGamepad(pad.index) : null)
              }}
            >
              Stop
            </Button>
          ...
          <p className="small muted">
            Firefox and Safari expose little or no rumble. Chrome and Edge are the most complete. On
            DualSense over Bluetooth, use Pro Mode rumble below if this does nothing.
          </p>
        </Card>
        <Card
          title="Trigger rumble"
          right={caps.trigger ? <Badge tone="good">supported</Badge> : <Badge>unsupported</Badge>}
        >
          <Slider
            label="Left trigger"
            value={lt}
          ...
          />
          <div className="row">
            <Button primary disabled={!caps.trigger} onClick={() => fire(0, 0, lt, rt)}>
              Play triggers
            </Button>
            <Button small disabled={!caps.trigger} onClick={() => fire(strong, weak, lt, rt)}>
              Play all four
            </Button>
          </div>
          <p className="small muted">
            Xbox impulse triggers via Chrome 126+ on Windows and macOS (Linux and ChromeOS over
            Bluetooth only). For DualSense adaptive triggers use Pro Mode.
          </p>
        </Card>
        {hid?.caps.rumble && (
          <Card
            title={`Rumble via WebHID (${hid.label})`}
            right={<Badge tone="accent">Pro Mode</Badge>}
          >
            <div className="row">
              <Button primary onClick={() => fireHid(strong, weak)}>
                Play
              </Button>
              {PRESETS.map((p) => (
                <Button key={p.label} small onClick={() => fireHid(p.strong, p.weak)}>
                  {p.label}
                </Button>
              ))}
              <Button small onClick={() => hid.rumble(0, 0)}>
                Stop
              </Button>
            </div>
            <p className="small muted">
              Writes the motor bytes directly, so it works on Bluetooth and in browsers whose
              Gamepad API rumble is missing.
            </p>
          </Card>
        )}
      </div>
