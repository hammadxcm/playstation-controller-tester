    ...
    <div className="grid-2">
      ...
      <Card title="Device">
        <dl className="kv">
          ...
          <dt>Family</dt><dd>{PROFILES[p.family].label}</dd>
          <dt>Vendor / product</dt><dd className="mono">{p.vid !== undefined ? `${p.vid.toString(16).padStart(4, '0')}:${p.pid?.toString(16).padStart(4, '0')}` : 'not exposed by this browser'}</dd>
          <dt>Mapping</dt><dd>{isStd ? <Badge tone="good">standard</Badge> : <Badge tone="ok">{pad.mapping || 'none'} — use the Learn tab</Badge>}</dd>
          <dt>Inputs</dt><dd>{gp ? `${gp.buttons.length} buttons, ${gp.axes.length} axes` : '–'}</dd>
          <dt>Rumble</dt><dd>{caps.dual ? <Badge tone="good">dual-rumble</Badge> : <Badge>none</Badge>} {caps.trigger && <Badge tone="good">trigger-rumble</Badge>}</dd>
          <dt>Raw id</dt><dd className="mono small dim">{pad.id}</dd>
        </dl>
      </Card>
    </div>
  )
