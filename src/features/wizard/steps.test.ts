...
const frame = (t: number, axes: number[], pressed: number[] = []): Frame => ({ index: 0, id: 'x', mapping: 'standard', t, hwT: t, axes, buttons: Array.from({ length: 18 }, (_, i) => ({ pressed: pressed.includes(i), value: pressed.includes(i) ? 1 : 0 })) })

describe('wizard steps', () => {
  it('builds six steps with sensible defaults', () => {
    const steps = buildSteps()
    expect(steps.map((s) => s.id)).toEqual(['drift', 'circle-left', 'circle-right', 'resolution', 'polling', 'buttons'])
  ...
  it('circularity steps complete after a full lap and flag short range', () => {
    ...
    for (let i = 0; i < 360; i++) left!.feed(frame(i, [Math.cos((i / 180) * Math.PI), Math.sin((i / 180) * Math.PI), 0, 0]))
    expect(left!.progress()).toBe(1)
    expect(left!.finish().rows[0]!.tone).toBe('good')
    for (let i = 0; i < 360; i++) right!.feed(frame(i, [0, 0, 0.8 * Math.cos((i / 180) * Math.PI), 0.8 * Math.sin((i / 180) * Math.PI)]))
  ...
  it('tolerates frames without axes or hardware timestamps, caps samples, grades partial results', () => {
    ...
    const bare: Frame = { index: 0, id: 'x', mapping: 'standard', t: 0, hwT: 0, axes: [], buttons: [] }
    drift!.feed(bare)
    left!.feed(bare)
    res!.feed(bare)
    poll!.feed(bare)
  ...
  it('resolution, polling and buttons steps produce metrics', () => {
    ...
    for (let i = 0; i < 256; i++) res!.feed(frame(i * 10, [(i / 255) * 2 - 1, (i / 255) * 2 - 1, 0, 0]))
