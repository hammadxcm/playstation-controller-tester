...
export function parseDualSense(
  d: DataView,
  off: number,
  ...
  return {
    ...
    touches: [
      decodeTouch(d, off + 32, TOUCHPAD.width, TOUCHPAD.height),
      decodeTouch(d, off + 36, TOUCHPAD.width, TOUCHPAD.height),
    ],
    gyro,
    accel,
    ...
    battery: {
      percent: Math.min((status & 0x0f) * 10 + 5, 100),
      state: BATTERY_STATE[st] ?? 'unknown',
    },
    flags: {
      headphones: !!(plug & 0x01),
      mic: !!(plug & 0x02),
      usb: !!(plug & 0x08) || !!(plug & 0x10),
    },
    extra: {
      seq: u8(6),
      r2Status: r2s >> 4,
