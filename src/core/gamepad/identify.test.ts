...
describe('identify', () => {
  it('parses Chromium ids', () => {
    const p = identify('DualSense Wireless Controller (STANDARD GAMEPAD Vendor: 054c Product: 0ce6)')
    expect(p).toEqual({ family: 'dualsense', name: 'DualSense Wireless Controller', vid: 0x054c, pid: 0x0ce6 })
  })
  it('parses Firefox ids, including unpadded legacy ones', () => {
    expect(identify('054c-09cc-Wireless Controller')).toMatchObject({ family: 'dualshock4', pid: 0x09cc })
    expect(identify('810-3-USB Gamepad')).toMatchObject({ family: 'generic', vid: 0x810, pid: 3, name: 'USB Gamepad' })
  })
