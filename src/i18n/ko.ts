...
export const ko: Dict = {
  ...
  hero: {
    ...
    title: '당신의 컨트롤러, GTA VI 준비됐나요?',
    lead: '미션을 망치기 전에 드리프트, 데드존, 닳은 버튼을 먼저 찾아내세요. 모든 버튼, 스틱, 트리거를 몇 초 만에 테스트. DualSense, DualSense Edge, DualShock 4, Xbox 및 모든 게임패드 지원. 설치도 계정도 필요 없습니다.',
    add: '기기 추가',
    pro: '프로 모드 열기',
    xbox: 'Xbox 컨트롤러',
  ...
  add: {
    ...
    step2: '프로 모드용 페어링',
    step2Body:
      'WebHID는 컨트롤러와 직접 통신합니다: 적응형 트리거, 라이트바, 플레이어 LED, 마이크 LED, 터치패드, 자이로, 배터리, 펌웨어. USB 또는 블루투스로 연결한 DualSense, DualSense Edge, DualShock 4와 블루투스로 연결한 Xbox 패드 지원.',
  ...
  xbox: {
    title: 'Xbox도 지원',
    lead: 'Xbox One S, Series X|S, Elite Series 2, Adaptive. 모든 화면은 Gamepad API로 동작하고, 프로 모드는 블루투스로 패드를 직접 읽습니다.',
    name: 'Xbox 무선 컨트롤러',
    tagline: 'One S · Series X|S · Elite Series 2 · Adaptive',
    callouts: ['임펄스 트리거', 'Xbox 버튼', '공유', 'Elite 2 패들'],
    facts: [
      '임펄스 트리거: 트리거마다 모터 내장',
      'WebHID로 블루투스 진동',
      '배터리와 충전 상태',
    ...
    matrix: {
      ...
      pro: '프로 모드 (블루투스)',
      buttons: '버튼, 스틱, 트리거',
      triggerRumble: '임펄스 트리거',
      battery: '배터리',
      paddles: 'Elite 2 패들, 프로필',
      raw: '원시 HID 리포트',
      chrome: 'Chrome / Edge',
      hires: '16비트 스틱',
    },
    note: 'USB로 연결된 Xbox 패드는 HID가 아닌 GIP로 통신하므로 프로 모드에는 블루투스가 필요합니다. 데스크톱 Chrome 또는 Edge에서 사용 가능하며, Windows에서는 Xbox 드라이버가 장치를 독점할 수 있습니다.',
  },
  tour: {
    ...
    pro: {
      title: '프로 모드',
      blurb: 'WebHID: 라이트바, LED, 마이크, 터치패드, 자이로, 배터리, Xbox 진동.',
    },
  ...
  pro: {
    title: '프로 모드 (WebHID)',
    body: 'Gamepad API가 닿지 않는 부분을 컨트롤러와 직접 통신합니다: 적응형 트리거, 라이트바, 플레이어 LED, 마이크 LED, 터치패드, 자이로와 가속도계, 배터리, 펌웨어, 공장 데이터. Chrome과 Edge에서 USB 또는 블루투스로 DualSense, DualSense Edge, DualShock 4를, 블루투스로 Xbox 패드를 지원합니다.',
    add: '기기 추가',
    hint: '아무것도 나타나지 않으면 Steam, PS Remote Play, DS4Windows, reWASD, Xbox 액세서리 앱을 종료하세요. 이들이 HID 리포트를 먼저 가져갑니다. 블루투스 패드는 축소 모드로 시작하며 앱이 자동으로 전체 리포트로 전환합니다.',
    noWebHid:
