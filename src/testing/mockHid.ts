...
export function createMockHid(family: string): MockHid {
  ...
  const state = (): HidState => {
    const gp = (window as unknown as { __ct: { gp: { axes: number[]; buttons: { value: number; pressed: boolean }[] } } }).__ct.gp
    const t = (performance.now() - t0) / 1000
    const b = gp.buttons
    const names = ds4
      ? ['cross', 'circle', 'square', 'triangle', 'l1', 'r1', 'l2', 'r2', 'share', 'options', 'l3', 'r3', 'up', 'down', 'left', 'right', 'ps', 'touchpad']
    ...
    return {
      ...
      extra: { seq: Math.floor(t * 250) & 0xff, l2Status: engaged.left ? 1 : 0, l2Engaged: engaged.left, r2Status: engaged.right ? 1 : 0, r2Engaged: engaged.right, micMuted: false },
      reportId: 1,
  ...
  const rec = (k: string) => async (...v: unknown[]) => {
    out[k] = v
  }
  return {
...
export async function installMockHid(family: string, q: URLSearchParams): Promise<MockHid> {
  if (!('hid' in navigator)) {
    Object.defineProperty(navigator, 'hid', {
      value: { addEventListener() {}, removeEventListener() {}, getDevices: async () => [], requestDevice: async () => [] },
      configurable: true,
    })
  }
  ...
  if (leds) {
    ...
    await hid.setPlayerLeds(m ? PLAYER_LED[Number(m[1]) - 1]! : Number(leds.replace('mask:', '')), 0)
  }
  const mic = q.get('mic')
