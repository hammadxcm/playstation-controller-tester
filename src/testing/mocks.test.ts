...
describe('mock HID', () => {
  it('produces synthetic state for DualSense and DualShock 4 and records commands', async () => {
    ...
    await ds4.close()
    const xb = createMockHid('xbox')
    expect(xb.family).toBe('xbox')
    expect(xb.caps.impulseTriggers).toBe(true)
    expect(xb.state().buttons.xbox).toBe(false)
    expect(xb.state().touches).toEqual([])
    expect((await xb.info()).layout).toBe('sparse')
    expect(await xb.factory!()).toEqual({})
    await xb.close()
    vi.useRealTimers()
  })
